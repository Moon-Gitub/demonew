...
def resolver_archivo_bases(preferido: str, alternativas: tuple[str, ...] = ()) -> str:
    """Devuelve el nombre del archivo de bases que exista en BASE_DIR."""
    for nombre in (preferido, *alternativas):
        if os.path.isfile(os.path.join(BASE_DIR, nombre)):
            return nombre
    return preferido
...
def main() -> None:
    ...
    print("\n--- Sistemas nuevos (bases2.txt / bases-2.txt) ---")
    bases_nuevos = resolver_archivo_bases(BASES_NUEVOS, ("bases-2.txt",))
    if not os.path.isfile(os.path.join(BASE_DIR, bases_nuevos)):
        print(f"❌ No se encontró bases2.txt ni bases-2.txt en {BASE_DIR}")
    elif bases_nuevos != BASES_NUEVOS:
        print(f"   (usando {bases_nuevos})")
