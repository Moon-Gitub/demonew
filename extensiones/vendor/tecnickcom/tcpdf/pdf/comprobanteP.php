...
$itemPedido = "codigo";
$respuestaVenta = ControladorVentas::ctrMostrarVentas($itemPedido, $_GET['codigo']);
	...
	<table border="1" style="padding: 10px">
		...
		<tr>
			...
			<td style="width:280px; font-size:10px; text-align: left;">
				...
				<span>Defensa al Consumidor Mza. 08002226678</span> 
...
for ($i = 0; $i <= 18; $i++) {
	...
	if($i < $tamanioProd){
		$getProducto = ControladorProductos::ctrMostrarProductoXId($productos[$i]["id"]);
		$datosFact[$i]["cantidad"] = $productos[$i]["cantidad"];
		$datosFact[$i]["descripcion"] = $productos[$i]["descripcion"];
		$unitario = $productos[$i]["total"] / (1 + ($getProducto["tipo_iva"] / 100));
		$datosFact[$i]["unitario"] = number_format($unitario,2,',','.');
		$datosFact[$i]["tipoIva"] = $getProducto["tipo_iva"];
		$datosFact[$i]["total"] = '$ ' . number_format($productos[$i]["total"],2,',','.');
	} else {

		$datosFact[$i]["cantidad"] = "";
		$datosFact[$i]["descripcion"] = "";
		$datosFact[$i]["unitario"] = "";
		$datosFact[$i]["tipoIva"] = "";
		$datosFact[$i]["total"] = "";
	}
}
...
$bloqueDatosFact .= '</td>
				...
				<tr>
					...
					<td style="width:120px; font-size:8px; text-align: rigth;  background-color: #f4f4f4">';
$bloqueDatosFact .= $neto_grav;
$bloqueDatosFact .= '</td>
				</tr>';

//Discrimino por los diferentes tipos de iva que tenga la venta
foreach ($ivas as $key => $value) {
$bloqueDatosFact .=	'<tr><td style="width:80px; font-size:8px; text-align: rigth;  background-color: #f4f4f4">';
$bloqueDatosFact .=	$value["descripcion"];
$bloqueDatosFact .= ': $</td>
					<td style="width:120px; font-size:8px; text-align: rigth;  background-color: #f4f4f4">';
$bloqueDatosFact .= $value["iva"];
$bloqueDatosFact .= '</td></tr>';
