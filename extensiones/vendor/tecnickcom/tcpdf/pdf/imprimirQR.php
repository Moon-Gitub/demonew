...
$jsonQRBase64 = $actual_link . '/index.php?ruta=precios-qr&idProducto=' . $producto["codigo"];

$pdf->write2DBarcode($jsonQRBase64, 'QRCODE,L', '', '', 40, 40, $style, 'N');
...
try {
    ...
    header('Content-Type: text/html; charset=utf-8');
    error_log("Error en imprimirQR: " . $e->getMessage());
    error_log("Stack trace: " . $e->getTraceAsString());
    die('Error al generar el PDF: ' . htmlspecialchars($e->getMessage()));
} catch (Error $e) {
    header('Content-Type: text/html; charset=utf-8');
    error_log("Error fatal en imprimirQR: " . $e->getMessage());
    error_log("Stack trace: " . $e->getTraceAsString());
